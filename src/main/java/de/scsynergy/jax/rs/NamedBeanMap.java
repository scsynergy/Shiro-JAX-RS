...
/**
 * Partial immutable {@link Map} implementation to provide a subset of CDI beans
 * as Shiro beans to be referenced in INI files. The keys are bean names, the
 * values are bean instances.
 * <p>
 * To be considered, a CDI bean must be qualified with {@link ShiroIni} and
 * optionally with {@code @Named}.
 * <p>
 * Contextual references of these beans are eagerly created when this class is
 * instantiated. This is the main reason for using the dedicated
 * {@code ShiroIni} qualifier, to avoid instantiating
...
public class NamedBeanMap implements Map<String, Object> {
    ...
    private static final Logger LOG = Logger.getLogger(NamedBeanMap.class.getName());
    ...
    public NamedBeanMap(BeanManager beanManager) {
        for (Bean<?> bean : beanManager.getBeans(Object.class, ShiroIniLiteral.INSTANCE)) {
            ...
            if (beanName == null) {
                LOG.log(Level.WARNING, "Shiro cannot derive a default name for [{}], "
                        + "so this bean cannot be referenced in shiro.ini. "
                        + "Consider adding a @Named qualifier.", bean);
            } else {
                LOG.log(Level.FINE, "Found @ShiroIni bean with name [{}]", beanName);
